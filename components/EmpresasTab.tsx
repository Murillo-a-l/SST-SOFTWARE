            ...
            <div className="flex flex-col md:flex-row gap-6 h-full">
                <Card className="md:w-1/3 flex flex-col">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <Button size="sm" onClick={onAdd} className="shadow-[0_10px_24px_rgba(192,121,84,0.18)]">
                            <AppIcon name="plus" className="h-4 w-4" /> Nova
                        </Button>
                    </div>
                    <div className="mb-4">
                        <Input
                            ...
                            className="w-full border-[#E3E8F2] bg-white/80 focus:ring-[#0F4C5C]/30 text-sm placeholder:text-xs"
                        />
                    </div>
                                    ...
                                    <button
                                        ...
                                        className={`w-full text-left p-2.5 border transition ${
                                            selectedEmpresa?.id === empresa.id
                                                ? 'bg-white border-[#C07954] text-[#0F4C5C]'
                                                : 'bg-white border-[#E3E8F2] hover:bg-white hover:border-[#C07954]/50'
                                        }`}
                                    >
                                        <span className="font-semibold text-sm">{empresa.nomeFantasia}</span>
                                    </button>
                                </li>
                            ))}
                        ...
                        <>
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex gap-2">
                                    <Button variant="secondary" size="sm" onClick={() => onEdit(selectedEmpresa)} className="shadow-[0_10px_22px_rgba(15,76,92,0.12)]">
                                        <AppIcon name="edit" className="h-4 w-4" /> Editar
                                    </Button>
                                    <Button variant="danger" size="sm" onClick={() => onDelete(selectedEmpresa)} className="bg-[#DC2626] shadow-[0_10px_22px_rgba(220,38,38,0.18)]">
                                        <AppIcon name="trash" className="h-4 w-4" /> Excluir
                                    </Button>
                                </div>
                            </div>
...
const InfoTab: React.FC<{empresa: Empresa}> = ({empresa}) => (
    <div className="space-y-4">
        ...
        <div className="pt-2">
            <h3 className="text-sm font-semibold text-slate-700 mb-2">Responsavel PCMSO</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
...
const DetailItem: React.FC<{label: string, value?: string | null}> = ({label, value}) => (
    <div className="space-y-1 border border-[#D5D8DC] p-3">
        <p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-slate-600">{label}</p>
...
const TabButton: React.FC<{name: DetailTab; label: string; activeTab: DetailTab; setActiveTab: (tab: DetailTab) => void;}> = ({ name, label, activeTab, setActiveTab }) => (
    <button
        ...
        className={`${
            activeTab === name
                ? 'bg-[#0F4C5C] text-white shadow-[0_10px_24px_rgba(15,76,92,0.18)]'
                : 'border border-[#E3E8F2] text-slate-600 hover:text-[#0F4C5C] hover:border-[#0F4C5C]/30'
        } whitespace-nowrap py-2.5 px-4 font-semibold text-sm transition`}
    >
        {label}
    </button>
);
